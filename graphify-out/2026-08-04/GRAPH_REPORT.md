...
## Corpus Check
- 32 files · ~21,149 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 209 nodes · 237 edges · 29 communities (16 shown, 13 thin omitted)
...
Cohesion: 0.17
Nodes (16): { db, runTransaction, serverTimestamp, getDoc, setDoc }, { executeInvest }, { Logger }, runDailyJob(), { sendAlert }, admin, db, getDoc() (+8 more)
...
Cohesion: 0.14
Nodes (20): axios, { Logger }, sendAlert(), stripEmoji(), claimDailyReward(), { Logger }, puppeteer, { claimDailyReward } (+12 more)
...
Cohesion: 0.10
Nodes (20): 1. Real-Time Transactions (Firestore: `users/{uid}/transactions/{id}`), 2. Categories (constant, local), 3. Simulation `manualIncome` (unchanged, localStorage), Architecture Overview, Data Models, File Summary, Future Enhancements (Out of Scope), Goal (+12 more)
...
Cohesion: 0.29
Nodes (7): 1. `index.html` — Remove auth overlay, add right panel structure, 2. `css/style.css` — Layout & Daily Log Panel Styles, 3. `js/app.js` — Core Orchestration, 4. `js/firebase.js` — Extended for Transactions, 5. `js/ui/dailyLog.js` — NEW FILE, 6. `js/engine/simulator.js` — UNCHANGED, File Changes

## Knowledge Gaps
- **127 isolated node(s):** `$schema`, `file:///D:/Investation/.kilo/plugins/graphify.js`, `snapshot`, `axios`, `{ Logger }` (+122 more)
...
- **Why does `File Changes` connect `File Changes` to `AGENTS.md`?**
